...
import { completeRegistration as pixelCompleteRegistration } from "@/helpers/pixel";
...
const LoginModal = ({ isOpen, onClose }) => {
  ...
  const isPhoneLoginAvailable = useSelector(
    (state) => state?.settings?.value?.system_settings?.[0]?.otp_login === "1"
  );
  ...
  const handleSendOTP = useCallback(async () => {
    ...
    try {
      const response = await verify_user({ mobile: state.phoneNumber.slice(2) });
  ...
  const handleResendOTP = useCallback(async () => {
    try {
      ...
      const response = await resend_otp({ mobile: state.phoneNumber.slice(2) });
