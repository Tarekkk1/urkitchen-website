          ...
          <>
            ...
            {selectedPayment === "paypal" && (
              <PayPalPayment
                ...
                finalTotal={rechargeAmount}
                amount={rechargeAmount}
                paymentMethodDetails={paymentMethodsMap["paypal"]}
                onClose={() => onOpenChange(false)}
                message={message}
              />
            )}
            {selectedPayment === "flutterwave" && (
              <FlutterwavePayment
                ...
                finalTotal={rechargeAmount}
                amount={rechargeAmount}
                paymentMethodDetails={paymentMethodsMap["flutterwave"]}
                onClose={() => onOpenChange(false)}
                message={message}
              />
            )}
            {selectedPayment === "phonepe" && (
              <PhonePePayment
                ...
                finalTotal={rechargeAmount}
                amount={rechargeAmount}
                paymentMethodDetails={paymentMethodsMap["phonepe"]}
                onClose={() => onOpenChange(false)}
                message={message}
              />
            )}
            {selectedPayment === "paymob" && (
              <PaymobPayment
                type="wallet"
                finalTotal={parseFloat(rechargeAmount)}
                paymentMethodDetails={paymentMethodsMap["paymob"]}
                onClose={() => onOpenChange(false)}
                cartStoreData={{ variant_id: [], data: [], overall_amount: 0 }}
                deliveryType="Delivery"
                selectedDeliveryAddress={null}
                deliveryChargesResponse={{ delivery_charge: 0 }}
                branch_id={null}
                is_self_pick_up={0}
              />
            )}
