...
import UserOrderCardSkeleton from "@/components/Skeleton/UserOrderCardSkeleton"; // Import skeleton
...
const UserOrderCard = ({
  ...
  const [cancelLoading, setCancelLoading] = useState(false); // New state for cancellation loading
  const [localStatus, setLocalStatus] = useState(status); // Local status state
  ...
  useEffect(() => {
    ...
    setLocalStatus(status); // Sync local status with prop
  }, [item, status]);
  ...
  const handleCancelOrder = async () => {
    setCancelLoading(true); // Show skeleton during cancellation
    try {
      const formData = new FormData();
      formData.append("order_id", id);
      ...
      const { data } = await api.post("/update_order_status", formData);
      if (data.error) {
        toast.error(data.message);
      } else {
        setLocalStatus("cancelled"); // Update local status immediately
        setOrderDetails((prev) => ({ ...prev, active_status: "cancelled" }));
        toast.success(data.message);
        onClose();
        request(); // Trigger parent refresh
      }
    } catch (error) {
      console.error(error);
      toast.error("An error occurred, please try again.");
    } finally {
      setCancelLoading(false); // Hide skeleton
    }
  };

  // Render skeleton if cancelLoading is true
  if (cancelLoading) {
    return <UserOrderCardSkeleton />;
  }
        ...
        <section className="flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div className="flex items-center gap-3">
              ...
              <Chip
                startContent={getStatusIcon(localStatus)} // Use localStatus
                color={
                  localStatus === "pending"
                    ? "warning"
                    : localStatus === "preparing" || localStatus === "confirmed"
                      ? "secondary"
            ...
            <div className="hidden sm:flex gap-2">
              {isCancellable === "1" &&
                [
                  ...
                  "preparing",
                  "out_for_delivery",
                ].includes(OrderDetails.active_status) &&
                OrderDetails.cancelable_till !== "" && (
                  <Button
                    variant="flat"
                    color="danger"
                    size="sm"
              ...
              <ModalContent>
                ...
                <ModalFooter>
                  ...
                  <Button
                    ...
                    color="danger"
                    onPress={handleCancelOrder} // Use new handler
                    className="rounded"
                  >
                    {cancelLoading ? t("please_wait") : t("yes")}
                  </Button>
                </ModalFooter>
              </ModalContent>
            </Modal>
