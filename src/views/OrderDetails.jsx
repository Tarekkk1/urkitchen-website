    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="w-full lg:w-2/3 space-y-6">
          ...
          {order?.active_status && order.active_status !== "cancelled" && (
            <Card className="rounded-lg">
              <CardHeader className="px-6 py-4">
                <h3 className="text-xl font-semibold">{t("order_tracking") || "Order Tracking"}</h3>
              </CardHeader>
              <CardBody className="px-6 pb-6">
                {(() => {
                  const steps = [
                    { key: "awaiting", label: t("order_placed") || "Order Placed" },
                    { key: "confirmed", label: t("confirmed") || "Confirmed" },
                    { key: "preparing", label: t("preparing") || "Preparing" },
                    { key: "out_for_delivery", label: t("out_for_delivery") || "Out for Delivery" },
                    { key: "delivered", label: t("delivered") || "Delivered" },
                  ];
                  const statusOrder = ["awaiting", "confirmed", "preparing", "out_for_delivery", "delivered"];
                  const currentIdx = statusOrder.indexOf(order.active_status);
                  return (
                        ...
                        return (
                          <div key={step.key} className="flex items-center flex-1 min-w-0">
                            <div className="flex flex-col items-center flex-shrink-0">
                              ...
                              <p className={`text-xs mt-1 text-center max-w-[64px] leading-tight
                                ${active ? "text-primary font-semibold" : done ? "text-gray-600" : "text-gray-400"}`}>
                                {step.label}
                              </p>
                            </div>
                            {idx < steps.length - 1 && (
