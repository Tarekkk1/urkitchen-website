...
import { setAddress as setDefaultCity } from "@/store/reducers/selectedMapAddressSlice";
import { addToCart as pixelAddToCart } from "../helpers/pixel";
...
export const add_to_cart = async ({
  ...
  if (qty === 0) {
    ...
    try {
      ...
      pixelAddToCart({ content_ids: [String(product_variant_id)] });
...
// Auto-select New Cairo as the default city on first load
export const autoSelectDefaultCity = async () => {
  // Skip if a branch is already stored (returning visitor or already selected)
  if (getBranchId()) return;

  try {
    const lat = process.env.NEXT_PUBLIC_LATITUDE || "30.0311";
    const lng = process.env.NEXT_PUBLIC_LONGITUDE || "31.4722";
    ...
    if (delivery.error || !delivery.data?.[0]?.branch_id) return;

    const branch_id = delivery.data[0].branch_id;

    store.dispatch(
      setDefaultCity({ city: "New Cairo", lat: Number(lat), lng: Number(lng) })
    );

    await changeBranchId({ branch_id });
  } catch {
    // Silent fail — user will see the cover page and can select manually
  }
};
